...
package net.ornithemc.loader.impl.metadata;
...
import net.ornithemc.loader.impl.lib.gson.JsonToken;
...
public final class ModMetadataParser {
	...
	private static LoaderModMetadata readModMetadata(InputStream is) throws IOException, ParseMetadataException {
		...
		// Furthermore we cannot guarantee the `schemaVersion` is the first field in every `ornithe.mod.json`
		//
		...
		try (JsonReader reader = new JsonReader(new InputStreamReader(is, StandardCharsets.UTF_8))) {
			...
			if (reader.peek() != JsonToken.BEGIN_OBJECT) {
				throw new ParseMetadataException("Root of \"ornithe.mod.json\" must be an object", reader);
			}
			...
			if (OrnitheLoader.getInstance().isDevelopmentEnvironment()) {
				Log.warn(LogCategory.METADATA, "\"ornithe.mod.json\" from mod %s did not have \"schemaVersion\" as first field.", ret.getId());
			}
	...
	private static LoaderModMetadata readModMetadata(JsonReader reader, int schemaVersion) throws IOException, ParseMetadataException {
		...
		default:
			if (schemaVersion > 0) {
				throw new ParseMetadataException(String.format("This version of ornithe-loader doesn't support the newer schema version of \"%s\""
						+ "\nPlease update ornithe-loader to be able to read this.", schemaVersion));
			}
