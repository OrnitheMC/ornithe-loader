...
final class V1ModMetadata extends AbstractModMetadata implements LoaderModMetadata {
	...
	public String getType() {
		return TYPE_ORNITHE_MOD; // Ornithe Mod
	}
	...
	public void emitFormatWarnings() {
		if (hasRequires) {
			Log.warn(LogCategory.METADATA, "Mod `%s` (%s) uses 'requires' key in ornithe.mod.json, which is not supported - use 'depends'", this.id, this.version);
		}
	}
