...
import net.ornithemc.loader.impl.gui.OrnitheStatusTree.OrnitheBasicButtonType;
import net.ornithemc.loader.impl.gui.OrnitheStatusTree.OrnitheStatusButton;
import net.ornithemc.loader.impl.gui.OrnitheStatusTree.OrnitheStatusNode;
import net.ornithemc.loader.impl.gui.OrnitheStatusTree.OrnitheStatusTab;
import net.ornithemc.loader.impl.gui.OrnitheStatusTree.OrnitheTreeWarningLevel;
import net.ornithemc.loader.impl.util.StringUtil;

class OrnitheMainWindow {
	static Icon missingIcon = null;

	static void open(OrnitheStatusTree tree, boolean shouldWait) throws Exception {
	...
	private static void open0(OrnitheStatusTree tree, boolean shouldWait) throws Exception {
	...
	private static void createUi(CountDownLatch onCloseLatch, OrnitheStatusTree tree) {
		JFrame window = new JFrame();
		window.setVisible(false);
		window.setTitle(tree.title);

		try {
			Image image = loadImage("/ui/icon/ornithe_x128.png");
		...
		if (tree.tabs.isEmpty()) {
			OrnitheStatusTab tab = new OrnitheStatusTab("Opening Errors");
			...
			OrnitheStatusTab tab = tree.tabs.get(0);
			contentPane.add(createTreePanel(tab.node, tab.filterLevel, icons), BorderLayout.CENTER);
		} else {
			JTabbedPane tabs = new JTabbedPane();
			contentPane.add(tabs, BorderLayout.CENTER);

			for (OrnitheStatusTab tab : tree.tabs) {
		...
		if (!tree.buttons.isEmpty()) {
			...
			for (OrnitheStatusButton button : tree.buttons) {
				JButton btn = new JButton(button.text);
				buttons.add(btn);
				btn.addActionListener(event -> {
					if (button.type == OrnitheBasicButtonType.CLICK_ONCE) btn.setEnabled(false);
	...
	private static JPanel createTreePanel(OrnitheStatusNode rootNode, OrnitheTreeWarningLevel minimumWarningLevel,
			IconSet iconSet) {
	...
	private static InputStream loadStream(String str) throws FileNotFoundException {
		InputStream stream = OrnitheMainWindow.class.getResourceAsStream(str);
	...
	static final class IconInfo {
		...
		public static IconInfo fromNode(OrnitheStatusNode node) {
			...
			OrnitheTreeWarningLevel warnLevel = node.getMaximumWarningLevel();

			if (split.length == 0) {
				// Empty string, but we might replace it with a warning
				if (warnLevel == OrnitheTreeWarningLevel.NONE) {
					main = "missing";
				} else {
	...
	static class CustomTreeNode implements TreeNode {
		...
		CustomTreeNode(TreeNode parent, OrnitheStatusNode node, OrnitheTreeWarningLevel minimumWarningLevel) {
			this.parent = parent;
			this.node = node;

			for (OrnitheStatusNode c : node.children) {
