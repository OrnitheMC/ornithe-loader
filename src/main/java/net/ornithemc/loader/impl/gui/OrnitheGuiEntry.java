...
/** The main entry point for all ornithe-based stuff. */
public final class OrnitheGuiEntry {
	/** Opens the given {@link OrnitheStatusTree} in a new swing window.
	 *
	 * @throws Exception if something went wrong while opening the window. */
	public static void open(OrnitheStatusTree tree) throws Exception {
		GameProvider provider = OrnitheLoaderImpl.INSTANCE.tryGetGameProvider();

		if (provider == null && LoaderUtil.hasAwtSupport()
				|| provider != null && provider.hasAwtSupport()) {
			OrnitheMainWindow.open(tree, true);
		} else {
			openForked(tree);
		}
	}

	private static void openForked(OrnitheStatusTree tree) throws IOException, InterruptedException {
		...
		Process process = new ProcessBuilder(javaPath.toString(), "-Xmx100M", "-cp", UrlUtil.LOADER_CODE_SOURCE.toString(), OrnitheGuiEntry.class.getName())
	...
	public static void main(String[] args) throws Exception {
		OrnitheStatusTree tree = new OrnitheStatusTree(new DataInputStream(System.in));
		OrnitheMainWindow.open(tree, true);
		System.exit(0);
	}
	...
	public static void displayError(String mainText, Throwable exception, boolean exitAfter) {
		displayError(mainText, exception, tree -> {
			...
			tree.addButton("Copy error", OrnitheBasicButtonType.CLICK_MANY).withClipboard(error.toString());
		}, exitAfter);
	}

	public static void displayError(String mainText, Throwable exception, Consumer<OrnitheStatusTree> treeCustomiser, boolean exitAfter) {
		GameProvider provider = OrnitheLoaderImpl.INSTANCE.tryGetGameProvider();

		if (!GraphicsEnvironment.isHeadless() && (provider == null || provider.canOpenErrorGui())) {
			String title = "Ornithe Loader " + OrnitheLoaderImpl.VERSION;
			OrnitheStatusTree tree = new OrnitheStatusTree(title, mainText);
			OrnitheStatusTab crashTab = tree.addTab("Crash");

			if (exception != null) {
				crashTab.node.addCleanedException(exception);
			} else {
				crashTab.node.addMessage("No further details available", OrnitheTreeWarningLevel.NONE);
			}

			// Maybe add an "open mods folder" button?
			// or should that be part of the main tree's right-click menu?
			tree.addButton("Exit", OrnitheBasicButtonType.CLICK_ONCE).makeClose();
