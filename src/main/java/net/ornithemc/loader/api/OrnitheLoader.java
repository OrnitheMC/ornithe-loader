...
package net.ornithemc.loader.api;
...
/**
 * The public-facing OrnitheLoader instance.
 *
 * <p>To obtain a working instance, call {@link #getInstance()}.</p>
 *
 * @since 0.4.0
 */
public interface OrnitheLoader {
	/**
	 * Returns the public-facing Ornithe Loader instance.
	 */
	static OrnitheLoader getInstance() {
		OrnitheLoader ret = OrnitheLoaderImpl.INSTANCE;

		if (ret == null) {
			throw new RuntimeException("Accessed OrnitheLoader too early!");
		}
	 ...
	 *
	 * @param key  the key in entrypoint declaration in {@code ornithe.mod.json}
	 ...
	 * Returns all entrypoints declared under a {@code key}, assuming they are of a specific type.
	 *
	 * <p>The entrypoint is declared in the {@code ornithe.mod.json} as following:
	 * <pre><blockquote>
	 *   "entrypoints": {
	 *     "&lt;a key&gt;": [
	 ...
	 * (only Java/game/Ornithe types like collections, primitives, String, Consumer, Function, ...).
	 *
	 ...
	 *
	 * @param id the ID of the mod, as defined in {@code ornithe.mod.json}
	 * @return whether or not the mod is present in this Ornithe Loader instance
	 */
	boolean isModLoaded(String id);

	/**
	 * Checks if Ornithe Loader is currently running in a "development"
